(() => {
  ...
  const certifications = [
    ...
    { label: 'Community Recognition', title: 'Microsoft Learn Student Ambassador', text: 'Recognition from my MLSA journey. The different certificates are grouped together instead of creating repetitive cards.', meta: 'Microsoft · MLSA', images: [
      `${A}community/Safiullah-Korai-Alpha-Microsoft-Student-Ambassador-Certificate.png`,
      `${A}community/Safiullah-Korai-Beta-Microsoft-Student-Ambassador-Certificate.png`,
      `${A}community/Safiullah-Korai-New-Microsoft-Student-Ambassador-Certificate.jpg`,
      `${A}community/Safiullah-Korai-Onboarding-Certificate-for-MLSA-YE.jpg`
    ] }
  ];
  ...
  const community = [
    {
      label: 'Led by me', title: 'Tafreeh-e-Data with Power BI',
      text: 'A two-day hands-on Power BI workshop I organized and led through my MLSA and Data Seekho Ambassador roles, with Fawad Khan as speaker and support from communities across Pakistan.',
      meta: 'Data Seekho × MLSA · 09–10 Nov',
      images: [
        `${G}Tafreeeh-e-Data With Power BI Speakers Safiullah Korai and Fawad Khan brought by Data Seekho X Microsoft Learn Student Ambassador_01.png`,
        `${G}Main Poster Tafreeeh-e-Data With Power BI Two Days Hands on workshop Presented by Data Seekho X Microsoft Learn Student Ambassador_02.jpg`,
        `${G}Tafreeh-e-Data With Power BI Supported by 7 Communities across Pakistan 03.jpg`
      ]
    },
    {
      label: 'Led by me', title: 'Winter Microsoft Learning',
      text: 'A 15-day Microsoft learning initiative organized by me through my MLSA role, delivered with ETL Online and supported by communities across Pakistan.',
      meta: 'MLSA × ETL Online · 15 days',
      images: [
        `${G}Main Poster Winter Microsoft Learning Organized by MLSA Safiullah Korai_01.jpg`,
        `${G}Poster Winter Microsoft Learning Organized by MLSA Safiullah Korai and Supported by 8 Communities Across Pakistan_02.jpg`,
        `${G}Winter Microsoft Learning Organized by MLSA Safiullah Korai and Sponsored by MLSA and ETl online_03.jpg`
      ]
    },
    {
      label: 'Led by me', title: 'Open Source Contribution Workshop',
      text: 'A hands-on open-source contribution workshop where I contributed as an MLSA and GSSoC Mentor/Ambassador, including the speaker session and event organization.',
      meta: 'GSSoC × MLSA · 2024',
      images: [
        `${G}OpenSourceContribution Main Poster.jpg`,
        `${G}Open Source Contribution Speaker Safiullah Korai.jpg`,
        `${G}Speaker Safiullah Korai for Open Source Ka Safar Organized by MLSA YE.jpg`
      ]
    },
    {
      label: 'Community Role', title: 'Microsoft Learn Student Ambassador',
      text: 'My MLSA role became a platform for organizing workshops, learning initiatives and cross-community collaborations rather than being only a credential.',
      meta: 'Microsoft · Student Ambassador',
      images: [
        `${A}community/Safiullah-Korai-New-Microsoft-Student-Ambassador-Certificate.jpg`,
        `${A}community/Safiullah-Korai-Alpha-Microsoft-Student-Ambassador-Certificate.png`,
        `${A}community/Safiullah-Korai-Beta-Microsoft-Student-Ambassador-Certificate.png`,
  ...
  const carousel = (images, title) => {
    ...
    return `<div class="achievement-carousel" data-carousel="${id}" aria-label="${esc(title)} photo carousel">
      <div class="achievement-carousel-track">${images.map((image, index) => `<img class="achievement-slide ${index === 0 ? 'is-active' : ''}" src="${esc(asset(image))}" alt="${esc(title)} — photo ${index + 1}" loading="lazy">`).join('')}</div>
      ${images.length > 1 ? `<button class="carousel-control carousel-prev" type="button" data-carousel-prev="${id}" aria-label="Previous photo">‹</button><button class="carousel-control carousel-next" type="button" data-carousel-next="${id}" aria-label="Next photo">›</button><div class="carousel-dots">${images.map((_, index) => `<button class="carousel-dot ${index === 0 ? 'is-active' : ''}" type="button" data-carousel-dot="${id}" data-slide="${index}" aria-label="Show photo ${index + 1}"></button>`).join('')}</div>` : ''}
    </div>`;
  };

  const card = (item, kind) => `<article class="card achievement-detail-card reveal">
    ${item.images ? carousel(item.images, item.title) : item.image ? `<div class="achievement-card-media"><img src="${esc(asset(item.image))}" alt="${esc(item.title)}" loading="lazy"></div>` : ''}
  ...
  function setupCarousels() {
    document.querySelectorAll('.achievement-carousel').forEach((carouselEl) => {
      const slides = [...carouselEl.querySelectorAll('.achievement-slide')];
      ...
      carouselEl.querySelector('[data-carousel-prev]')?.addEventListener('click', () => show(current - 1));
      carouselEl.querySelector('[data-carousel-next]')?.addEventListener('click', () => show(current + 1));
